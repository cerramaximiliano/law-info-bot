    ...
    lines.forEach((line, index) => {
      ...
      if (/Hora[:\s]*\$?/i.test(line) && /Mensual[:\s]*\$?/i.test(line)) {
        ...
        if (horaMatch && mensualMatch) {
          const horaValue = parseFloat(horaMatch[1].replace(/\./g, "").replace(",", "."));
          const mensualValue = parseFloat(mensualMatch[1].replace(/\./g, "").replace(",", "."));
          numbers.push({
      ...
      if (!processed) {
        let modifiedLine = line.replace(regex, (match) => {
          ...
          if (previousLine.match(/Hora[:\s]*\$?/i) || line.match(/Hora[:\s]*\$?/i)) {
            numbers.push({
              caracter: currentCaracter,
              tipo: "hora",
              valor: value,
