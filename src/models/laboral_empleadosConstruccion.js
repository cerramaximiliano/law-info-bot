...
const LaboralConstruccionSchema = new Schema({
  fecha: {
    type: Date,
    required: true,
    unique: true,
...
LaboralConstruccionSchema.pre('save', function(next) {
  this.ultimaActualizacion = new Date();
  next();
});

// Índices
LaboralConstruccionSchema.index({ fecha: 1 });
LaboralConstruccionSchema.index({ 'detalles.categoría': 1 });

// Métodos estáticos
LaboralConstruccionSchema.statics.encontrarPorFecha = function(fecha) {
  return this.findOne({ fecha: new Date(fecha) });
};

LaboralConstruccionSchema.statics.encontrarPorCategoria = function(categoria) {
  return this.find({
...
LaboralConstruccionSchema.set('toJSON', {
  virtuals: true,
...
const LaboralConstruccion = mongoose.model('LaboralConstruccion', LaboralConstruccionSchema);

module.exports = LaboralConstruccion;