...
function iterateTextByLine(text) {
  ...
  lines.forEach((line, index) => {
    ...
    if (retiroMatch) {
      //console.log(`Encontrado: ${retiroMatch[0]}`);
    ...
    // Encontrar y procesar líneas que contienen "Hora: $" y "Mensual: $" en la misma línea
/*     if (/Hora: \$\s*\d+/i.test(line) && /Mensual: \$\s*\d+/i.test(line)) {
      const cleanedLine = line.replace(/[\$:]/g, "").trim();
      const [horaStr, mensualStr] = cleanedLine
        .split("Mensual")
        .map((str) => str.trim());
      ...
      const mensualValue = parseFloat(
        mensualStr.replace(/\.(?=\d{3})/g, "").replace(",", ".")
      );
      numbers.push({
    ...
    let modifiedLine = line.replace(regex, (match) => {
      ...
      if (previousLine.includes("Hora: $") || line.includes("Hora: $")) {
        ...
        previousLine.includes("Mensual: $") ||
        line.includes("Mensual: $")
      ) {
        numbers.push({
          caracter: currentCaracter,
          tipo: "mensual",
  ...
  //console.log(groupedNumbers, groupedNumbers.length);
  ...
  if (categoryIndex === categoryMatches.length && groupedNumbers.length > 0) {
    logger.info("Éxito: Todas las categorías fueron asignadas correctamente.");
