const LaboralGastronomia = require("../models/laboral_empleadosGastronomicos")

const saveGastronomia = async (datos) => {
    try {
        ...
        try {
            ...
            const nuevaActualizacion = new LaboralGastronomia(actualizacion);
            // Guardar
            ...
            if (error.code === 11000) {
                const actualizacionExistente = await LaboralGastronomia.findOneAndUpdate(
...
const findByDateGastronomia = async (date, type = "day") => {
    try {
        ...
        const actualizaciones = await LaboralGastronomia.find({
            fecha: {
                $gte: startOfDay,
                $lte: endOfDay,
            },
...
const updateGrastronomiaById = async (id, updateData) => {
    try {
        ...
        const actualizacion = await LaboralGastronomia.findByIdAndUpdate(
            id,
            updateData,
            {
...
module.exports = 
{ saveGastronomia, findByDateGastronomia, updateGrastronomiaById}