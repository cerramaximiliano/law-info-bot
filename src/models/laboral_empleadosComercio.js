...
const LaboralComercioSchema = new Schema({
  fecha: {
    type: Date,
    required: true,
    unique: true,
...
LaboralComercioSchema.pre('save', function(next) {
  this.ultimaActualizacion = new Date();
  next();
});

// Índices
LaboralComercioSchema.index({ fecha: 1 });
LaboralComercioSchema.index({ 'detalles.categoría': 1 });

// Métodos estáticos
LaboralComercioSchema.statics.encontrarPorFecha = function(fecha) {
  return this.findOne({ fecha: new Date(fecha) });
};

LaboralComercioSchema.statics.encontrarPorCategoria = function(categoria) {
  return this.find({
...
LaboralComercioSchema.set('toJSON', {
  virtuals: true,
...
const LaboralComercio = mongoose.model('LaboralComercio', LaboralComercioSchema);

module.exports = LaboralComercio;